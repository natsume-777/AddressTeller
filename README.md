...
> **注意（現状の制限）**: これらの設定値は実装上 `EditorPrefs` に保存されており、**マシン単位**（同じマシン上の全プロジェクトで共有・チームメンバー間では共有されません）。Project Settings 画面に表示されますが、`.unitypackage` やリポジトリ経由でチームに配布されるものではありません。プロジェクト共有の設定として永続化する対応は今後予定しています。
