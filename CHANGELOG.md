...
- `AddressRuleBuilderImpl`/`RuleEvaluator`/`RuleExplanation`/`RuleMatchOutcome`/`RuleEvaluationDetail` を internal 化。内部実装の可視性を整理し、ユーザー公開 DSL 面（`AddressRuleBase`/`IAddressRuleBuilder`/`Match`/`Naming`/`AssetContext`）への影響なし。
