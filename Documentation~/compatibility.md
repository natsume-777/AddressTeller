...
**`SchemaVersion`**: currently `1` (`AddressTellerReportBuilder.CurrentSchemaVersion`). Incremented whenever a shape change above would otherwise be silently backward-incompatible for a consumer parsing the JSON structurally (as opposed to just reading new optional fields). The package itself does not reject an unrecognized `SchemaVersion` when reading a report back in (there is currently no CLI path that re-reads a report file), but a value higher than the consumer understands should be treated as "I don't know this shape" by any external tooling that parses these reports.
...
**`SchemaVersion`**: currently `1` (`AddressTellerSnapshotService.CurrentSchemaVersion`). Unlike the report format, snapshot loading actively enforces this: `LoadFromFile` rejects a file whose `SchemaVersion` is greater than the currently-supported value. A value of `0` means "older-format JSON that predates this field, or an uninitialized instance" and is accepted.
